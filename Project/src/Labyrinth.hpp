...
void DescriptorSet::init(BaseProject *bp, DescriptorSetLayout *DSL,
	...
	for (size_t i = 0; i < BP->swapChainImages.size(); i++) {
		...
		// persistent storage for the infos so pointers remain valid
		std::vector<VkDescriptorBufferInfo> bufferInfos(E.size());
		std::vector<VkDescriptorImageInfo> imageInfos(E.size());
		...
		for (size_t j = 0; j < E.size(); j++) {
			...
			if (E[j].type == UNIFORM) {
				// fill persistent buffer info
				bufferInfos[j].buffer = uniformBuffers[j][i];
				bufferInfos[j].offset = 0;
				bufferInfos[j].range = E[j].size;
				...
				imageInfos[j].imageView = E[j].tex->textureImageView;
				imageInfos[j].sampler = E[j].tex->textureSampler;
				...
				// handle other descriptor types if any
				descriptorWrites[j].pBufferInfo = nullptr;
				descriptorWrites[j].pImageInfo = nullptr;
				descriptorWrites[j].pTexelBufferView = nullptr;
			}
		...
		// Now pointers are valid because bufferInfos/imageInfos live here
		vkUpdateDescriptorSets(BP->device,
			static_cast<uint32_t>(descriptorWrites.size()),
			descriptorWrites.data(), 0, nullptr);
	}
